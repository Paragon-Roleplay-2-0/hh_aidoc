---@diagnostic disable: lowercase-global

if not lib.checkDependency('ox_lib', '3.30.0', true) then return end

local QBCore = exports['qb-core']:GetCoreObject()

local Active = false

local test = nil
local test1 = nil
local spam = true

local function log(text)
	TriggerServerEvent('qb-log:server:CreateLog', 'hh_aidoc', 'Medic Command', 'default', text)
end

RegisterCommand("medic", function (source, args, raw)
	local PlayerData = QBCore.Functions.GetPlayerData()
	local characterInfo = PlayerData.charinfo
	if (QBCore.Functions.GetPlayerData().metadata["isdead"]) or (QBCore.Functions.GetPlayerData().metadata["inlaststand"]) and spam then
		lib.callback('hhfw:docOnline', false, function(EMSOnline, hasEnoughMoney)
			if EMSOnline <= Config.Doctor and hasEnoughMoney and spam then
				SpawnVehicle(GetEntityCoords(PlayerPedId()))
				TriggerServerEvent('hhfw:charge')
				lib.notify({
					description = 'The EMT Is Arriving!',
					duration = 5000,
					position = 'center-right',
					type = 'success'
				})
			elseif not hasEnoughMoney then
				lib.notify({
					description = 'You Don\'t Have Enough Money!',
					duration = 5000,
					position = 'center-right',
					type = 'error'
				})
			else
				lib.notify({
					description = 'Wait! The EMT Is On Their Way!',
					duration = 5000,
					position = 'center-right',
					type = 'inform'
				})
			end
		end)
	else
		lib.notify({
			description = 'You Can Only Use This Command When You\'re Dead!',
			duration = 5000,
			position = 'center-right',
			type = 'error'
		})
	end
	log('Player ' .. characterInfo.firstname .. ' ' .. characterInfo.lastname .. ' used this command to revive themselves IC!')
end, false)

function SpawnVehicle(x, y, z)
	spam = false
	local vehhash = GetHashKey("nkstxems") -- "ambulance"
	local loc = GetEntityCoords(PlayerPedId())
	RequestModel(vehhash)
	while not HasModelLoaded(vehhash) do
		Wait(1)
	end
	RequestModel('s_f_y_scrubs_01')
	while not HasModelLoaded('s_f_y_scrubs_01') do
		Wait(1)
	end
	local spawnRadius = 40
	local found, spawnPos, spawnHeading = GetClosestVehicleNodeWithHeading(
	loc.x + math.random(-spawnRadius, spawnRadius), loc.y + math.random(-spawnRadius, spawnRadius), loc.z, 0, 3, 0)

	if not DoesEntityExist(vehhash) then
		mechVeh = CreateVehicle(vehhash, spawnPos, spawnHeading, true, false)
		ClearAreaOfVehicles(GetEntityCoords(mechVeh), 5000, false, false, false, false, false);
		SetVehicleOnGroundProperly(mechVeh)
		SetVehicleNumberPlateText(mechVeh, "HHFW")
		SetEntityAsMissionEntity(mechVeh, true, true)
		SetVehicleEngineOn(mechVeh, true, true, false)

		mechPed = CreatePedInsideVehicle(mechVeh, 26, GetHashKey('s_f_y_scrubs_01'), -1, true, false)

		mechBlip = AddBlipForEntity(mechVeh)
		SetBlipFlashes(mechBlip, true)
		SetBlipColour(mechBlip, 5)


		PlaySoundFrontend(-1, "Text_Arrive_Tone", "Phone_SoundSet_Default", 1)
		Wait(2000)
		TaskVehicleDriveToCoord(mechPed, mechVeh, loc.x, loc.y, loc.z, 20.0, 0, GetEntityModel(mechVeh), 524863, 2.0)
		test = mechVeh
		test1 = mechPed
		Active = true
	end
end

CreateThread(function()
	while true do
		Wait(200)
		if Active then
			local loc = GetEntityCoords(GetPlayerPed(-1))
			local lc = GetEntityCoords(test)
			local ld = GetEntityCoords(test1)
			local dist = Vdist(loc.x, loc.y, loc.z, lc.x, lc.y, lc.z)
			local dist1 = Vdist(loc.x, loc.y, loc.z, ld.x, ld.y, ld.z)
			if dist <= 10 then
				if Active then
					TaskGoToCoordAnyMeans(test1, loc.x, loc.y, loc.z, 1.0, 0, 0, 786603, 0xbf800000)
				end
				if dist1 <= 1 then
					Active = false
					ClearPedTasksImmediately(test1)
					DoctorNPC()
				end
			end
		end
	end
end)

function DoctorNPC()
	RequestAnimDict("mini@cpr@char_a@cpr_str")
	while not HasAnimDictLoaded("mini@cpr@char_a@cpr_str") do
		Wait(1000)
	end

	TaskPlayAnim(test1, "mini@cpr@char_a@cpr_str", "cpr_pumpchest", 1.0, 1.0, -1, 9, 1.0, false, false, false)

	if lib.progressCircle({
		duration = Config.ReviveTime,
		position = 'bottom',
		label = 'The EMT Is Giving You Medical Aid...',
		useWhileDead = false,
		canCancel = false,
		disable = {
			move = false,
			car = false,
			combat = true,
			mouse = false,
		}
	}) then
		ClearPedTasks(test1)
		Wait(500)
		-- TriggerEvent("hospital:client:Revive") -- QBCore Default
		TriggerEvent('wasabi_ambulance:revive') -- Wasabi Ambulance
		StopScreenEffect('DeathFailOut')
		lib.notify({
			title = 'Treatment Completed!',
			description = 'You Were Charged: ' .. Config.Price,
			duration = 5000,
			position = 'center-right',
			type = 'success'
		})
		RemovePedElegantly(test1)
		DeleteEntity(test)
		Wait(5000)
		DeleteEntity(test1)
		spam = true
	end
end